...
![深夜画廊 · 选人界面(WASM 实机截图)](./img/vibe-coding-game-dev/gallery-select-final.png)

*选人界面:三位证人并排立在夜画廊里,透明立绘干净地叠在光影背景上;顶部是案情提要,底部提示开始问询。*

![深夜画廊 · 审讯中(WASM 实机截图)](./img/vibe-coding-game-dev/gallery-interro-final.png)
...
根因挖出来:voice 参数写错了字段(`config.parameters.voice_id`),平台处理器实际读的是 **`config.voiceId`**(camelCase、数组形式)。修正字段 + 原生 pitch 后再次测量:**291 / 202 / 158 Hz**,少女/播报/御姐三个真实音色(PR #15、#17)。
...
注意我说的是**现象**和"系统性"三个字,没有指定方案。agent 把整个引擎的音频重构成**三条声道**(PR #20):SFX 可叠但帧内去重、Music 同名不重启、Voice 播新台词先掐旧的——对话永不重叠。补丁能修一次,模型能修一类,这次重构惠及全部 12 个游戏。
...
|---|---|---|
| 立绘 | 文生图底图 + 图生图表情 | `assets/textures/vg_*.png` 同名同尺寸,`set_sprite_image` 一行切表情 |
| 抠图 | removeBackground(BiRefNet) | 透明 PNG 直接叠加,无需引擎改动 |
| 语音 | minimax TTS,voice_id 固定进角色表 | **Voice 声道**:`play_voice` 先掐后播,对话永不重叠 |
| 氛围乐 | 文生音乐(Lyria 2) | **Music 声道**:`play_music` 同名不重启,场景重入不炸音 |
| 剧情 | —(agent 直接写分支树,LLM 即编辑器) | Lua 热重载秒级验证;`dest` 分支跳转纯数据可 diff |
...
- **Web / WASM**:ottavino(纯 Rust Lua VM)驱动,暴露一模一样的 `game` API;连"浏览器音频要首次点击解锁"这种平台差异,也被引擎层吸收(index.html 里 resume AudioContext),脚本毫无感知。
