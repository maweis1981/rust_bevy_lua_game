...
# --- Lua emit ---------------------------------------------------------------
def to_lua(name, pattern, grid, tray, w, h):
    pal = PALETTES[pattern]
    lines = []
    lines.append(f"-- generated by tools/gen_level.py --pattern {pattern}")
    lines.append(f"{name} = {{")
...
def main():
    ...
    sys.stderr.write(
        f"[gen_level] pattern={args.pattern} {args.w}x{args.h} painted={total} "
        f"colours={len(PALETTES[args.pattern])} batches={len(tray)} "
        f"peak_frontier_colours={peak} (>= recommended slots) "
        f"solvable@{args.slots}slots={ok} replay_steps={steps}\n"
    )
    if not ok:
        sys.exit("VALIDATION FAILED: level is not solvable")
    if not args.check:
        print(to_lua(args.name, args.pattern, grid, tray, args.w, args.h))
