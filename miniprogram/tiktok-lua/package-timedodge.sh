...
echo "  US  (revenue = IAP, no ads) -> index.html: set clientKey + __IAP_ENDPOINT"
echo "       project.config.json: US appid · deploy server/ (see server/README.md) and"
echo "       add its domain to the app's trusted request domains + webhook."
echo "  ROW (revenue = rewarded ads) -> index.html: set clientKey · project.config.json: ROW appid"
